\section{Math Fundamentals}
	...
	\subsubsection{Some Indefinite Integrals}
	...
	\subsection{Differential Calculus}
	  ...
	  \frac{f(x_0+\Delta)x-f(x_0)}{\Delta x}$\\
		\begin{tabular}{llll}
			...
			\quad or \quad $\displaystyle\frac{d f(g(x))}{dx} = f'(g(x)) \cdot g'(x)$\\[0.1cm] Product rule:	&
			$\left(f(x)\cdot g(x)\right)'$ &$=$ & $f'(x)\cdot g(x) + f(x)\cdot g'(x)$\\
			[0.1cm] Quotient rule:& $\displaystyle\frac{f(x)}{g(x)}$ &$=$ & $\displaystyle\frac{f'(x)g(x)-f(x)g'(x)}{g^2(x)}$\\
		\end{tabular}
